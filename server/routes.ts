...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(`${apiPrefix}/admin/users`, requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post(`${apiPrefix}/admin/users`, requireAuth, requireAdmin, async (req, res) => {
    try {
      const { username, password, isAdmin, canManagePeriods, canManageEvents, canManageFigures, canManageSites } = req.body;
      ...
      // Tạo người dùng mới với thông tin phân quyền
      const userData = {
        username, 
        password,
        isAdmin: isAdmin === true,
        canManagePeriods: canManagePeriods === true,
        canManageEvents: canManageEvents === true,
        canManageFigures: canManageFigures === true,
  ...
  app.put(`${apiPrefix}/admin/users/:id`, requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const updatedUserData = {
        ...
        canManagePeriods: userData.canManagePeriods,
        canManageEvents: userData.canManageEvents,
        canManageFigures: userData.canManageFigures,
        canManageSites: userData.canManageSites
      };
  ...
  app.delete(`${apiPrefix}/admin/users/:id`, requireAuth, requireAdmin, async (req, res) => {
    try {
      const userId = parseInt(req.params.id);
