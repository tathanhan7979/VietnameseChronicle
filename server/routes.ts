...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(`${apiPrefix}/search`, async (req, res) => {
    try {
      ...
      // Track search
      await storage.trackSearch(term ? String(term) : undefined);
  ...
  app.get(`${apiPrefix}/admin/stats`, requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      res.json({
        ...
        visitsCount: await storage.getVisitsCount(),
        searchCount: await storage.getSearchesCount()
      });
    } catch (error) {
  ...
  app.use(async (req, res, next) => {
    ...
    // Track visit for all other requests
    if (!url.startsWith(`${apiPrefix}/admin`)) {
      await storage.trackVisit();
    }
