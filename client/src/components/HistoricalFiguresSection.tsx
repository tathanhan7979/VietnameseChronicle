...
export default function HistoricalFiguresSection({
  ...
  const { data: queryPeriods, isLoading: isLoadingPeriods } = useQuery<
    PeriodData[]
  >({
  ...
  const [randomizedFigures, setRandomizedFigures] = useState<
    HistoricalFigure[]
  >([]);
  ...
  if (
    (isLoadingFigures && !propFigures) ||
    (isLoadingPeriods && !propPeriods)
  ) {
    return (
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="font-['Playfair_Display'] font-bold text-3xl md:text-4xl text-[#4527A0] inline-flex items-center justify-center gap-3">
            ...
            >
              Những Nhân Vật{" "}
              <span className="text-[#C62828]">Lịch Sử Tiêu Biểu</span>
            </span>
          </h2>
          <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
            Tìm hiểu về những nhân vật đã đóng góp và định hình nền lịch sử hào
          ...
          {figures.slice(0, visibleCount).map((figure, index) => (
            ...
            >
              <div className="relative">
                ...
                <Link
                  href={`/thoi-ky/${periods.find((p) => p.id === figure.periodId)?.slug || "unknown"}`}
                >
                ...
                <div className="mt-auto pt-3 border-t border-gray-100 mt-4">
                  <Link href={`/nhan-vat/${figure.id}/${slugify(figure.name)}`}>
                    <div className="flex justify-between items-center group cursor-pointer">
                      <span className="text-[#4527A0] font-medium text-sm group-hover:underline">
                        Xem chi tiết
                      </span>
