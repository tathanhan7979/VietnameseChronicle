...
export default function ContributorsBasic() {
  ...
  const handleSubmit = async (values: ContributorFormValues) => {
    ...
    try {
      ...
      // Refresh the list with a small delay to ensure server has processed the change
      setTimeout(fetchContributors, 300);
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Refresh the list with a delay to ensure server has processed the deletion
      setTimeout(fetchContributors, 300);
    } catch (error) {
      ...
      <div className="container mx-auto py-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="space-x-2">
            ...
            <Button variant="outline" onClick={fetchContributors}>
              <Loader2 className={isLoading ? "animate-spin h-4 w-4 mr-2" : "h-4 w-4 mr-2"} /> Làm mới
            </Button>
          </div>
        </div>
