    ...
    >
      <div className="flex items-center gap-3 flex-1">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-medium text-gray-800 dark:text-white truncate">{site.name}</h3>
          <div className="flex items-center gap-1">
            <span className="text-xs text-gray-500 dark:text-gray-400 truncate">
              {periods.find(p => p.id === site.periodId)?.name || "Chưa phân loại"} • {site.location}
            </span>
            {site.mapUrl && (
              <a 
                href={site.mapUrl} 
...
const HistoricalSitesContext = createContext<HistoricalSitesContextType>({ periods: [] });
...
export default function HistoricalSitesAdmin() {
  ...
  const [relatedEventId, setRelatedEventId] = useState<number | undefined>(undefined);
  ...
  const updateSortOrder = async (orderedIds: number[]) => {
    try {
      const response = await apiRequest('POST', '/api/admin/historical-sites/reorder', { orderedIds });
    ...
    try {
      ...
      if (isRemovingImage) {
        ...
        if (isEditing && currentSite) {
          ...
          siteData.imageUrl = previewImage; // This would be temporary until proper upload
        }
      } else if (imageUrl) {
      ...
      if (isEditing && currentSite) {
        ...
        response = await apiRequest('PUT', `/api/admin/historical-sites/${currentSite.id}`, siteData);
        successMessage = 'Cập nhật địa danh lịch sử thành công';
      } else {
        // Create new site
        response = await apiRequest('POST', '/api/admin/historical-sites', siteData);
      ...
      // Refresh sites list
      const refreshResponse = await apiRequest('GET', '/api/admin/historical-sites');
  ...
  const handleDeleteSite = async () => {
    ...
    try {
      const response = await apiRequest('DELETE', `/api/admin/historical-sites/${siteToDelete.id}`);
      const result = await response.json();
    ...
    <AdminLayout title="Quản lý địa danh lịch sử">
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-lg shadow p-6 text-center">
          <p className="text-gray-500 dark:text-gray-400">Chưa có địa danh lịch sử nào.</p>
        ...
        <HistoricalSitesContext.Provider value={{ periods }}>
          ...
          <div className="grid gap-6">
            {periods.map(period => {
              ...
              const periodSites = sites.filter(site => site.periodId === period.id);
      ...
      <Dialog open={currentSite !== null} onOpenChange={handleCloseDialog}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Điền thông tin chi tiết về địa danh lịch sử. Các trường có dấu * là bắt buộc.
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn xóa địa danh "{siteToDelete?.name}"? 
