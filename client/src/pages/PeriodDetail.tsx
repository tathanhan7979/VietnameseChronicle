...
import { ArrowLeft, CalendarDays, Clock, History, Home, MapPin, List } from 'lucide-react';
...
export default function PeriodDetail() {
  ...
  const { data: period, isLoading: isLoadingPeriod, error: periodError } = useQuery<PeriodData>({
  ...
  const { data: allFigures, isLoading: isLoadingFigures } = useQuery<HistoricalFigure[]>({
  ...
  const figures = allFigures?.filter(figure => 
    period && figure.period.includes(period.name)
  ) || [];
  
  const isLoading = isLoadingPeriod || isLoadingEvents || isLoadingFigures || isLoadingSites;
  
  // If period not found
  if (!isLoading && !period) {
    return (
      <div className="container mx-auto px-4 py-12 min-h-screen flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold text-[hsl(var(--primary))] mb-4">Không tìm thấy thời kỳ</h1>
        <p className="mb-8">Thời kỳ lịch sử bạn đang tìm kiếm không tồn tại hoặc đã bị xóa.</p>
        <Link href="/">
  ...
  if (!period || periodError) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <p className="mb-6">Không thể tải thông tin thời kỳ. Vui lòng thử lại sau.</p>
  ...
  const eventsContent = (!events || events.length === 0) ? (
    <div className="text-center py-10">
      <div className="text-gray-500 mb-4">
        <List className="h-16 w-16 mx-auto opacity-25" />
      </div>
      <h3 className="text-xl font-semibold mb-2">Chưa có sự kiện</h3>
      <p className="text-gray-600">Chưa có sự kiện nào được thêm vào thời kỳ này.</p>
    </div>
  ) : (
        ...
        <motion.div 
          ...
          <div className="p-6">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-xl font-bold mb-2 text-[hsl(var(--primary))]">{event.title}</h3>
  ...
  const figuresContent = (!figures || figures.length === 0) ? (
    <div className="text-center py-10">
      <div className="text-gray-500 mb-4">
      ...
      <p className="text-gray-600">Chưa có nhân vật lịch sử nào được thêm vào thời kỳ này.</p>
    </div>
  ) : (
      ...
      {figures.map((figure, index) => (
        <motion.div 
          ...
          <div className="h-48 overflow-hidden relative">
            <img 
              src={figure.imageUrl || 'https://via.placeholder.com/400x250?text=Nhân+Vật+Lịch+Sử'} 
              alt={figure.name} 
  ...
  const sitesContent = (!sites || sites.length === 0) ? (
    <div className="text-center py-10">
      <div className="text-gray-500 mb-4">
        <MapPin className="h-16 w-16 mx-auto opacity-25" />
      </div>
      <h3 className="text-xl font-semibold mb-2">Chưa có di tích lịch sử</h3>
      <p className="text-gray-600">Chưa có di tích lịch sử nào được thêm vào thời kỳ này.</p>
    </div>
  ) : (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-6">
      {sites.map((site, index) => (
        <motion.div 
          ...
          <div className="h-48 overflow-hidden relative">
            <img 
              src={site.imageUrl || 'https://via.placeholder.com/400x250?text=Di+Tích+Lịch+Sử'} 
              alt={site.name} 
              className="w-full h-full object-cover transition-transform hover:scale-105"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end">
          ...
          <div className="p-4">
            ...
            <div className="mt-4 flex justify-between gap-2">
              {site.mapUrl && (
                <Button variant="outline" size="sm" className="flex-1" onClick={() => window.open(site.mapUrl, '_blank')}>
                  <MapPin className="mr-2 h-4 w-4" />
                  Bản đồ
                </Button>
              )}
              <Link href={`/di-tich/${site.id}/${slugify(site.name)}`} className="flex-1">
    ...
    <div className="bg-gradient-to-b from-white to-gray-50 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--secondary))] py-16 text-white">
        <div className="container mx-auto px-4">
          <Link href={`/#timeline?period=${period.slug}`}>
            <Button variant="outline" className="mb-6 text-black border-white bg-white/90 hover:bg-white hover:text-primary">
          ...
          <div className="flex items-center mb-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold font-['Playfair_Display'] mb-2">{period.name}</h1>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h2 className="text-xl font-semibold mb-4 text-[hsl(var(--primary))]">Tổng quan</h2>
