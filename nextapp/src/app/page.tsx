...
// Thiết lập metadata tĩnh cho trang chủ
...
async function fetchPeriodsData(): Promise<PeriodData[]> {
  try {
    const res = await fetch('http://localhost:5000/api/periods', { next: { revalidate: 3600 } });
...
async function fetchEventsData(): Promise<EventData[]> {
  try {
    const res = await fetch('http://localhost:5000/api/events', { next: { revalidate: 3600 } });
...
async function fetchHistoricalFigures(): Promise<HistoricalFigure[]> {
  try {
    const res = await fetch('http://localhost:5000/api/historical-figures', { next: { revalidate: 3600 } });
...
async function fetchHistoricalSites(): Promise<HistoricalSite[]> {
  try {
    const res = await fetch('http://localhost:5000/api/historical-sites', { next: { revalidate: 3600 } });
...
export default async function Home() {
  const periods = await fetchPeriodsData();
  const events = await fetchEventsData();
  const figures = await fetchHistoricalFigures();
  const sites = await fetchHistoricalSites();
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-screen flex items-center">
        ...
        <div className="absolute inset-0 z-0">
          <Image
            src="https://lichsuviet.edu.vn/uploads/banner-image.png"
