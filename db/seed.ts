...
<p>Đặc biệt, tại đây có 82 bia đá khắc tên các tiến sĩ qua các kỳ thi từ năm 1442 đến 1779, được đặt trên lưng rùa đá, tượng trưng cho sự trường thọ và bền vững của học vấn.</p>`,
    ...
    const newsData: schema.InsertNews[] = [
      {
    ...
    // Update period_id for news based on existing periods
    try {
      // Get period data
      ...
      if (periods.length > 0) {
        ...
        const hienDaiPeriod = periods.find(p => p.name.includes('Hiện đại') || p.name.includes('1945'));
        
        if (lyTranPeriod) {
          // Tin tức về Văn Miếu, Hoàng thành Thăng Long
          newsData[0].period_id = lyTranPeriod.id;
          newsData[3].period_id = lyTranPeriod.id;
        }
        
        if (leSoPeriod) {
          // Tin tức về Khởi nghĩa Lam Sơn
          newsData[1].period_id = leSoPeriod.id;
        }
        
        if (hienDaiPeriod) {
          // Tin tức về Điện Biên Phủ
          newsData[4].period_id = hienDaiPeriod.id;
        }
      }
    } catch (error) {
      console.error("Error updating period_id for news:", error);
    }

    // Insert news data
    for (const news_item of newsData) {
      const existingNews = await db.query.news.findFirst({
        where: eq(schema.news.slug, news_item.slug)
      });
      
      if (!existingNews) {
        await db.insert(schema.news).values(news_item);
        console.log(`Added news: ${news_item.title}`);
      } else {
        console.log(`News already exists: ${news_item.title}`);
      }
    }
